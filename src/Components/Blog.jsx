...
import MalenaIcon from '../images/male_icon.png';
import CarouselComponent from './CarouselComponent.jsx';
import MaleLogo from '../images/logomale.png';
import Formulario from './Formulario';
import FormularioItems from './FormularioItems';
import FormularioNewItem from './FormularioNewItem';
...
function Blog(props) {
  ...
  const [objMalenaDescription, setObjMalenaDescription] = useState({});
  ...
  const [mostrarFormularioItems, setMostrarFormularioItems] = useState(false);

  const [mostrarFormularioNewItem, setMostrarFormularioNewItem] = useState(false);

  const [nroPaginaActual, setNroPaginaActual] = useState(0);
  ...
  const handleGetComment = async () => {
    try {
      const { data } = await axios.get(`${API_URL}/api/comentarios/`, {
        params: {
          orden: 'desc'
        }
      });
  ...
  const handleAddComment = async (comment) => {
    try {
      await axios.post(`${API_URL}/api/comentarios/`, comment);
      await handleGetComment();
    } catch (error) {
      alert("Oops ocurrió un error inesperado. Intente nuevamente!");
    }
  ...
  const handleDeleteComment = async (idToDelete) => {
    try {
      ...
      await axios.delete(`${API_URL}/api/comentarios/${idToDelete}`, config);
      await handleGetComment();
    } catch (error) {
      alert("Oops ocurrió un error inesperado. Intente nuevamente!");
    }
  ...
  const handleGetDescription = async () => {
    try {
      const { data } = await axios.get(`${API_URL}/api/descripcion/`);
      setObjMalenaDescription(data[0]);
    } catch (error) {
      alert("Oops ocurrió un error inesperado. Intente nuevamente!");
    }
  };

  const handleUpdateDescription = async (newObj) => {
    try {
      const config = {
        headers: { Authorization: `Bearer ${token}` }
      };
      const idToUpdate = objMalenaDescription._id;
      await axios.put(`${API_URL}/api/descripcion/${idToUpdate}`, newObj, config);
      await handleGetDescription();
    } catch (error) {
      alert("Oops ocurrió un error inesperado. Intente nuevamente!");
    }
  ...
  const handleGetItems = async () => {
    try {
      ...
      const dataItems = data.map((item) => {
        const fullDescriptionArr = dividirCadena(item.fullDescription);
        return { ...item, fullDescriptionArr };
      });
      setItems(dataItems);
    } catch (error) {
      alert("Oops ocurrió un error inesperado. Intente nuevamente!");
  ...
  const handleUpdateItems = async (newItem, idToUpdate) => {
    try {
      const config = {
        headers: { Authorization: `Bearer ${token}` }
      };
      await axios.put(`${API_URL}/api/items/${idToUpdate}`, newItem, config);
      await handleGetItems();
    } catch (error) {
      alert("Oops ocurrió un error inesperado. Intente nuevamente!");
    }
  ...
  const handlePostItem = async (newItem) => {
    try {
      ...
      await axios.post(`${API_URL}/api/items/`, newItem, config);
      await handleGetItems();
    } catch (error) {
      alert("Oops ocurrió un error inesperado. Intente nuevamente!");
    }
  ...
  const handleDeleteItem = async (idToDelete) => {
    try {
      ...
      await axios.delete(`${API_URL}/api/items/${idToDelete}`, config);
      await handleGetItems();
    } catch (error) {
      alert("Oops ocurrió un error inesperado. Intente nuevamente!");
    }
  ...
  const handleSalirModoAdmin = () => {
    setIsAdmin(false);
    setToken("");
    props.handleLogout();
  };

  const HandlerClickNewItem = () => {
    setMostrarFormularioNewItem(!mostrarFormularioNewItem);
  };

  const HandlerClickCard = () => {
    setMostrarFormularioCard(!mostrarFormularioCard);
  };

  const HandlerClickItems = () => {
    setMostrarFormularioItems(!mostrarFormularioItems);
  };

  const HandlerClick = () => {
    setNroPaginaActual(0);
    setClicked(!clicked);
  };

  const handleIndexChange = (indexItem) => {
    setIndexChange(indexItem);
  };

  const handleNroPagina = (arg) => {
    if (arg === "atras") {
      if (nroPaginaActual !== 0) {
        setNroPaginaActual(nroPaginaActual - 1);
      }
    } else {
      if (nroPaginaActual !== items[indexChange].fullDescriptionArr.length - 1) {
        setNroPaginaActual(nroPaginaActual + 1);
      }
    }
  };
  ...
  function dividirCadena(texto) {
    ...
    for (let i = 0; i < texto.length; i++) {
      ...
      if ((flagCTE && texto[i] === ".") || (i === texto.length - 1)) {
        arrayTexto.push(texto.slice(start, i + 1).trimStart());
        start = i + 1;
        flagCTE = false;
      }
    }
      ...
      <>
        {clicked ? (
          <div className='container d-flex justify-content-center align-items-center w-100 vh-100'>
            <div className="card2">
              <h1>{items[indexChange].title}</h1> <br />
              <h3>{new Date(items[indexChange].date).toLocaleDateString()}</h3> <br />
              <p>{items[indexChange].fullDescriptionArr[nroPaginaActual]}</p> <br />
              <div className='botonesPagina'>
                <button onClick={() => handleNroPagina("atras")} type="button" className="btn btn-lg btn-light">←Anterior</button>
                <p>Página {nroPaginaActual + 1}/{items[indexChange].fullDescriptionArr.length}</p>
                <button onClick={() => handleNroPagina("adelante")} type="button" className="btn btn-lg btn-light">Siguiente→</button>
              </div>
              <button onClick={HandlerClick} className='botonVolverAtras'> Volver atrás
          ...
          <>
            {isAdmin && mostrarFormularioCard ? (
              <div className='container'>
                < Formulario onSubmitClick={handleUpdateDescription} objMalenaDescription={objMalenaDescription} OnClickCancel={HandlerClickCard} />
              </div>
            ) : (
              isAdmin && mostrarFormularioItems ? (
                <div className='container d-flex justify-content-center align-items-center flex-column'>
                  < FormularioItems items={items} OnClickCancel={HandlerClickItems} onSubmitClick={handleUpdateItems} onDeleteClick={handleDeleteItem} />
                </div>
              ) : (isAdmin && mostrarFormularioNewItem ? (
                <div className='container d-flex justify-content-center align-items-center flex-column'>
                  < FormularioNewItem OnClickCancel={HandlerClickNewItem} onSubmitClick={handlePostItem} />
                </div>
              ) : ( 
                <div className='container contenedor-principal'>
                  {isAdmin ? (<button onClick={() => {handleSalirModoAdmin()}} type="button" className="btn btn-lg btn-warning">Salir de modo Administrador</button>) : (<Link className="btn btn-lg btn-link" to="/login">Administrar</Link>)}
                  <div className='row'>
                    <img className='logo-princ' src={MaleLogo} alt="logo de malena" />
                  </div>
                  <div className="divider"><span></span><span><i className="bi bi-book"></i></span><span></span></div>
                  <div className='row secc2'>
                    <div className="page">
                      <div className="margin"></div>
                      {isAdmin ? (
                        <button className="buttonEdit" onClick={HandlerClickCard}>
                      ...
                      <h1>{objMalenaDescription.titulo}</h1>
                      <p>{objMalenaDescription.texto}
                      </p>
                    </div>
